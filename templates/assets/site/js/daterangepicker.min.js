! function (t, e) {
  ...
  s.prototype = {
    ...
    renderCalendar: function (t) {
      ...
      var C = "left" == t ? this.minDate : this.startDate,
        b = this.maxDate,
        Y = ("left" == t ? this.startDate : this.endDate, '<table class="table-condensed">');
      Y += "<thead>", Y += "<tr>", this.showWeekNumbers && (Y += "<th></th>"), Y += C && !C.isBefore(e.firstDay) || this.linkedCalendars && "left" != t ? "<th></th>" : '<th class="prev available"><i class="month_move_icon">&lt;</i></th>';
      var P = this.locale.monthNames[e[1][1].month()] + e[1][1].format(" YYYY");
      if (this.showDropdowns) {
        for (var w = e[1][1].month(), M = e[1][1].year(), x = b && b.year() || M + 5, I = C && C.year() || M - 50, L = M == I, A = M == x, _ = '<select class="monthselect">', S = 0; 12 > S; S++) _ += (!L || S >= C.month()) && (!A || S <= b.month()) ? "<option value='" + S + "'" + (S === w ? " selected='selected'" : "") + ">" + this.locale.monthNames[S] + "</option>" : "<option value='" + S + "'" + (S === w ? " selected='selected'" : "") + " disabled='disabled'>" + this.locale.monthNames[S] + "</option>";
        _ += "</select>";
        for (var B = '<select class="yearselect">', E = I; x >= E; E++) B += '<option value="' + E + '"' + (E === M ? ' selected="selected"' : "") + ">" + E + "</option>";
        B += "</select>", P = _ + B
      }
      if (Y += '<th colspan="5" class="month">' + P + "</th>", Y += b && !b.isAfter(e.lastDay) || this.linkedCalendars && "right" != t && !this.singleDatePicker ? "<th></th>" : '<th class="next available"><i class="month_move_icon">&gt;</i></th>', Y += "</tr>", Y += "<tr>", this.showWeekNumbers && (Y += '<th class="week">' + this.locale.weekLabel + "</th>"), i.each(this.locale.daysOfWeek, function (t, e) {
          Y += "<th>" + e + "</th>"
